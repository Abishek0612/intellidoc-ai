...
import { Search, Sparkles, BookOpen, Clock, Copy } from "lucide-react";
...
const ResearchPage: React.FC = () => {
  ...
  }, [query, searchHistory.length, suggestions.length]);

  const handleSearch = useCallback(async (): Promise<void> => {
    if (!query.trim()) {
      toast.warning("Please enter a search query", {
        position: window.innerWidth < 768 ? "top-center" : "top-right",
        toastId: "search-warning",
      });
      return;
    }
    ...
    try {
      ...
      dispatch({
        ...
        payload: {
          ...researchDocument,
          id: searchItem.id,
          lastModified: searchItem.timestamp,
        },
      });
  ...
  }, [query, generate, searchHistory, dispatch]);
  ...
  const handleHistoryClick = useCallback((item: SearchItem): void => {
    setQuery(item.query);
  ...
  const suggestionItems = useMemo(
    () =>
      suggestions.map((suggestion, index) => ({ id: index, text: suggestion })),
    [suggestions]
  );
      ...
      <div className="bg-white border-b border-gray-200 p-4 flex-shrink-0">
        ...
        <div className="flex gap-2 relative" ref={suggestionsRef}>
          ...
          <button
            ...
            className="px-6 py-3 bg-blue-500 text-white rounded-lg hover:bg-blue-600 disabled:bg-gray-300 transition-colors flex items-center gap-2 flex-shrink-0"
          >
            {loading ? (
